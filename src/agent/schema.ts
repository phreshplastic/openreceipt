import { z } from "zod";
import { defaultWordmarkStyleId } from "../blocks/wordmarks";
...
export const draftBlockSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("logo"), name: z.string().min(1).max(40), tagline: z.string().max(40).optional() }),
]);
...
export async function compileDraftBlock(block: DraftBlock, dependencies?: CatalogDependencies): Promise<ReceiptBlock> {
  switch (block.type) {
    ...
    case "logo":
      // No `mark` here on purpose. The wordmark is the printer's identity, set once
      // in personalisation; letting an agent restyle it per receipt makes a person's
      // own shelf of paper look like it came from six different shops.
      return catalog("logo", { style: defaultWordmarkStyleId, primary: block.name, secondary: block.tagline }, dependencies);
  }
}
