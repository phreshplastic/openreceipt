...
import { ArrowLeft, ArrowRight, Check, MapPin, RefreshCw, Usb, WifiOff } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { Brand } from "../components/Brand";
import { getCapabilities, saveConfiguration, type BridgeCapabilities, type PaperProfile } from "../bridge/client";

type HomeDefaults = { location: string; unit: "fahrenheit" | "celsius" };
type Props = {
  defaults: HomeDefaults;
  onComplete(profile: PaperProfile, defaults: HomeDefaults): Promise<void>;
  onTestPrint(profile: PaperProfile): Promise<void>;
};

export function SetupPage({ defaults, onComplete, onTestPrint }: Props) {
  ...
  // Guessed from the browser's timezone; nothing was asked of the user and no IP was sent anywhere.
  const [home, setHome] = useState<HomeDefaults>(defaults);
  const probe = async () => {
    setLoading(true);
    setMessage("");
  ...
  const configure = async () => { setMessage("Saving printer…"); try { await saveConfiguration(profileId); await onComplete(profile, home); navigate("/app"); } catch (error) { setMessage(error instanceof Error ? error.message : "Setup failed."); } };
  ...
  return <main className="setup-page"><header className="setup-top"><Brand compact /><button className="text-button" onClick={() => navigate("/")}><ArrowLeft size={14} />Back</button></header><section className="setup-card">
    ...
    <div className="setup-row"><div className="setup-row-icon"><MapPin size={20} /></div><div className="setup-row-copy"><strong>Where you are</strong><span>Blocks that fetch weather or air quality start here. You can change it on any block.</span></div><div className="setup-home">
      <input aria-label="Home city" placeholder="City or postal code" value={home.location} onChange={(event) => setHome({ ...home, location: event.target.value })} />
      <select aria-label="Temperature units" value={home.unit} onChange={(event) => setHome({ ...home, unit: event.target.value as HomeDefaults["unit"] })}><option value="fahrenheit">°F</option><option value="celsius">°C</option></select>
    </div></div>
